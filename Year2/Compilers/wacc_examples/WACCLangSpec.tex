...
% I think in here we should paste all language keywords i.e:
% 'begin' 'end' 'skip' 'read' 'free' 'return' 'exit' 'print' 'println'
% 'if' 'then' 'else' 'fi' 'fst' 'snd' 'pair'
% And also type names:
% 'int' 'bool' 'char' 'string' 
% And also operators:
% '!' '-' 'len' 'ord' 'chr'
% ^ there is a lot more -> we should ask if we need to list them all.
Variable names must not clash with \hl{reserved keywords, such as any type or statement.}

They can consist 
\hl{
  of a sequence of characters starting with any alphabetic character (lower- or uppercase)
...
A variable must be declared before \hl{it is used}
...
In this case \hl{the variable is overridden within that scope.}

Once the child scope is exited \hl{the variable re-assumes it's old value.}
...
\hl{Exit statements are a keyword used to exit the program and return to the OS an appropriate exit code. They are implemented by the pattern: exit $\langle expr \rangle$, where $\langle expr \rangle$ is the exit code.  }
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{C|L|C|C}
    ...
    \hline
    \lit*{string} or \lit*{char[]} & \hl{Outputs the contents of the string or array as a string on stdout.} & \hl{$[‘h’,’i’,’!’]$} & \hl{"hi!"} \\
    \hline
    Other Array Types & \hl{Prints the memory address at the head of the array as a hexadecimal string.} & \hl{$[1,2,3]$} & \hl{``0x22150''} \\  
    \hline
    \lit*{pair} & \hl{Prints the memory address of the pair as a hexadecimal string} & \lit*{newpair(a, b)}\footnotemark[1] & \hl{``0x22150''} \\
    \hline
  \end{tabulary}
...
\hl{While loop statements create a repeated section of code represented by $\langle stat \rangle$ that repeat while an $\langle expr \rangle$ holds true. They are formed with the following pattern: while $\langle expr\rangle$ do $\langle stat \rangle$ done.
}
...
\hl{A program is comprised of several statements, which effectively are instructions for the computer. Each statement contains either two other statements separated by a semicolon ($\langle stat \rangle ; \langle stat \rangle$), that are executed in order, an instruction that yields to a other statement(s) (such as if $\langle expr \rangle$ then $\langle stat\rangle$ else $\langle stat \rangle$) or a singleton instruction such as an assignment or system call. Using either of the former two, statements can be chained together to form more complicated programs by forming a tree of sorts.
}
...
\hl{An expression is said to be side effect free if none of the constituent variables are altered by their presence in the expression. }
...
\begin{table}
  ...
  \begin{tabular}{cclc}
    ...
    \hl{\textbackslash b} & \lit*{0x08} & \hl{backspace} & \hl{BS} \\
    \hl{\textbackslash t} & \lit*{0x09} & \hl{tab} & \hl{HT}\ \\
    \hl{\textbackslash n} & \lit*{0x0a} & \hl{new line} & \hl{LF} \\
    \hl{\textbackslash f} & \lit*{0x0c} & \hl{form feed} & \hl{FF} \\
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCL}
    ...
    \lit{ord} & \hl{char} & \hl{int} & \hl{ASCII Order} \\
    \lit{chr} & \hl{int} & \hl{char} & \hl{Character of ASCII Value} \\
    \hline
  \end{tabulary}
...
\item The \lit{ord} operator \hl{ returns the returns the ASCII value of the provided character as an \texttt{int} type.} \fillgap{\hl{Define/describe the {\lit{ord}} operator}}{1 mark}

\item The \lit{chr} operator \hl{returns the character representation of the ASCII value of int in the subexpression (which can be a literal or an expression that evaluates to an \texttt{int}).} \fillgap{\hl{Define/describe the {\lit{chr}} operator}}{1 mark} 
...
\begin{table}
  ...
  \begin{tabulary}{\textwidth}{CCCCCL}
    ...
    \lit{>} & 3 & \hl{char, int} & \hl{Same as type of Argument 1} & \hl{bool} & Greater Than \\
    \lit{>=} & 3 & \hl{char, int} & \hl{Same as type of Argument 1} & \hl{bool} & Greater Than or Equal \\
    \lit{<} & 3 & \hl{char, int} & \hl{Same as type of Argument 1} & \hl{bool} & Less Than \\
    \lit{<=} & 3 & \hl{char, int} & \hl{Same as type of Argument 1} & \hl{bool} & Less Than or Equal \\
    \lit{==} & 4 & \hl{any} & \hl{Same as type of Argument 1} & \hl{bool} & Equality \\
    \lit{!=} & 4 & \hl{any} & \hl{Same as type of Argument 1} & \hl{bool} & Inequality \\
...
\hl{Comments are strings of text that get ignored by the compiler. In WACC, comments on a given line are defined as text after the first \# sign on that line. \\
Comments can be defined on a separate line, where \# is the first character on the line, or in-line with code to be compiled, where \# is added after the code.\\
All text after the first \# will be seen as a comment, including respective \# signs, so comments cannot be placed between code segments on the same line. \\
WACC also only recognizes single-line comments. For multi-line comments, \# must precede every line.}
