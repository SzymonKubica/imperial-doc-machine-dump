...
opTable 
  ...
  where
    bool f a b = if f a b then 1 else 0 

apply :: Op -> Int -> Int -> Int
apply op a b  
  = (lookUp op opTable) a b  
...
execStatement w@ (DoWhile b p) s 
  | eval p s' == 1 = execStatement w s'
  where
    s' = execBlock b s 
...
foldConst (Apply op (Const c) (Const c'))
  = Const (apply op c c') 
foldConst (Apply op e@(Var _) (Const 0))
  = e
foldConst (Apply op (Const 0) e@(Var _))
  = e
foldConst e 
  = e
    ...
    scan' (Assign v' e) (wl, b)
      ...
      | otherwise
        = case (sub v n e) of
            ...
            e'      -> (wl, Assign v' e' : b) 
    scan' (If p q r) (wl , b) 
      = (wl ++ wl' ++ wl'', If (sub v n e) q' r' : b) 
      where
        (wl', q') = scan v n q
        (wl'', r') = scan v n r
    scan' (DoWhile body p) (wl, b) 
      = (wl ++ wl', DoWhile body' (sub v n p)  
      where
