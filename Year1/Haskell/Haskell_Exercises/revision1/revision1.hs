...
execStatement d@(DoWhile b e) s
  | eval e s' == 1 = execStatement d s' 
  | otherwise      = s'
  where
    s' = execBlock b s
...
execBlock [] s
  = s
execBlock (st : b) s 
  = execBlock b (execStatement st s)
...
foldConst ph@(Phi (Const c) (Const c')) 
  | c == c' = Const c
  | otherwise = ph 
...
sub v n e 
  ...
  where
    sub' v n var@(Var v') 
      | v == v' = Const n
      | otherwise = var
    sub' v n (Apply op e1 e2)
      = Apply op (sub v n e1) (sub v n e2)
    sub' v n (Phi e1 e2) 
      = Phi (sub v n e1) (sub v n e2) 
    sub' v n (Const c) 
      = Const c
...
scan v n b
  = scan' ([], b)
  where
    scan' :: (Worklist, Block) -> (Worklist, Block)
    scan' (wl, []) = (wl, [])
    scan' (wl, ((Assign v' e) : b))
      | v == v' = scan'(wl, (Const n : b) 
      | otherwise = 

scan2 :: (Exp -> Exp) -> Block -> (Exp -> Exp, Block)
scan2 = undefined
propagateConstants :: Block -> Block
-- Pre: the block is in SSA form
propagateConstants b
  = snd (propCon (scan "$INVALID" 0 b))
  where
    propCon :: (Worklist, Block) -> (Worklist, Block)
    propCon ((v, n) : wl, b)
      = propCon (wl'', b')
      where
        (wl', b') = scan v n b
